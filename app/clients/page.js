...
function FeeHistory({ history }) {
  ...
  if (!ketoan.length && !log.length) return null
...
export default function ClientsPage() {
  ...
  const loadFeeHistory = async (clientId) => {
    ...
    try {
      ...
      const json = await res.json()
      setFeeHistory(h => ({ ...h, [clientId]: json.error ? { ketoan: [], changeLog: [] } : json }))
    } catch (_) {
      setFeeHistory(h => ({ ...h, [clientId]: { ketoan: [], changeLog: [] } }))
    }
  }
      ...
      <div className="px-4 md:px-8 py-5">
        ...
        <div className="space-y-3">
          {filtered.map(client => {
            ...
            const history = feeHistory[client.id] || { ketoan: [], changeLog: [] }
