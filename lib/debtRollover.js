...
export async function ensureRollovers(supabase, clientIds, currentYear, currentMonth) {
  ...
  const [{ data: clients }, { data: existingRollovers }, { data: feesKetoan }] = await Promise.all([
    supabase.from('clients').select('id, monthly_fee, other_debt, created_at').in('id', clientIds),
