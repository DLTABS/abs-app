...
// Khi áp một mức phí mới "từ tháng X trở đi", có được cập nhật luôn clients.monthly_fee (phí
// SỐNG hiện tại) không?
//
// CHỈ khi tháng X là mốc phí MỚI NHẤT. Nếu đã tồn tại mốc fee_plan ở tháng SAU X thì đây là lần
// áp phí LÙI để sửa lại một giai đoạn cũ — phí hiện tại phải giữ nguyên theo mốc mới nhất kia.
// Ghi đè sẽ làm phí hiện tại tụt về mức cũ, và client_change_log ghi một lần "đổi phí" không có
// thật (dòng đó còn được resolveFeeForMonth dùng làm lớp fallback -> tính sai phí tháng cũ).
//
// existingPlans: mảng fee_plan đã có của công ty, dạng [{year, month}].
export function shouldUpdateLiveFee(existingPlans, targetYear, targetMonth) {
  const target = Number(targetYear) * 12 + Number(targetMonth)
  return !(existingPlans || []).some(p => Number(p.year) * 12 + Number(p.month) > target)
}
